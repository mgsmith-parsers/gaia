...
/**
 * Tests for AI (243) — PCN.
 *
 * <p>Packaging component number
 *
...
class Ai243Test {
    ...
    /** A well-formed value for AI (243). */
    ...
    private static final String VALID_INPUT =
            ...
            + GS1Constants_AICodes.AI_243_PCN + "A";

    @Test
    @DisplayName("AI (243) with a valid value parses and is valid")
    void validElementParsed() {
        ParseResult resp = parser.parse(VALID_INPUT);
        assertTrue(resp.isValid(), "A well-formed AI (243) element string must be valid");
        assertNotNull(resp.getAiObject().get(GS1Constants_AICodes.AI_243_PCN), "AI (243) element must be present");
        assertEquals(VALID_VALUE, resp.getAiObject().get(GS1Constants_AICodes.AI_243_PCN).getValue(),
                "Parsed value must equal the input value");
    }

    @Test
    @DisplayName("AI (243) with no value produces an error")
    void missingValueProducesError() {
        ParseResult resp = parser.parse(GS1Constants_AICodes.AI_243_PCN);
        assertFalse(resp.isValid(),
                "Parsing AI (243) with no value must produce at least one error");
    }

    @Test
    @DisplayName("AI (243) with maximum-length value is valid")
    void maximumLengthValueIsValid() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_01_GTIN + "09506000134352"
                + GS1Constants_AICodes.AI_243_PCN + "A".repeat(20));
        assertTrue(resp.isValid(), "A maximum-length value (20 chars) must be valid");
        assertEquals("A".repeat(20), resp.getAiObject().get(GS1Constants_AICodes.AI_243_PCN).getValue(),
                "Parsed value must equal the maximum-length input");
    }

    @Test
    @DisplayName("AI (243) with over-maximum-length value produces an error")
    void overMaximumLengthValueProducesError() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_01_GTIN + "09506000134352"
                + GS1Constants_AICodes.AI_243_PCN + "A".repeat(21));
        assertFalse(resp.isValid(),
                "A value longer than the declared maximum (21 chars) must produce an error");
    }

    @Test
    @DisplayName("AI (243) with an out-of-charset character produces an error")
    void invalidCharacterProducesError() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_01_GTIN + "09506000134352"
                + GS1Constants_AICodes.AI_243_PCN + "#");
        assertFalse(resp.isValid(),
                "A value containing a character outside the AI's character set must produce an error");
    }

    @Test
    @DisplayName("AI (243) without its required AIs produces GE-S005")
    void missingRequiredAisProducesError() {
        ParseResult resp = parser.parse(GS1Constants_AICodes.AI_243_PCN + VALID_VALUE);
        assertFalse(resp.isValid(),
                "AI (243) without its required AIs must produce a required-AI error");
        assertTrue(resp.getErrors().stream().anyMatch(e -> "GE-S005".equals(e.getId())),
                "The error must be GE-S005 (missing required AI)");
    }

    @Test
    @DisplayName("AI (243) terminated by a trailing FNC1 is valid with an advisory warning")
    ...
    @Test
    @DisplayName("Fixed-length AI (01) followed by FNC1 then AI (243) produces a syntax error")
    void fixedLengthAiWithFnc1BeforeThisAiProducesError() {
        // A FNC1 after fixed-length AI (01) mid-stream triggers GE-S011.
        // The parser stops at the FNC1 and AI (243) is never reached.
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_01_GTIN + "09506000134352"
                + GS1Constants.FNC1_GS
                + GS1Constants_AICodes.AI_243_PCN + "A");
