...
class Ai3902Test {
    ...
    /** A well-formed value for AI (3902). */
    ...
    private static final String VALID_INPUT =
            ...
            + GS1Constants_AICodes.AI_3902_AMOUNT + "1";

    @Test
    @DisplayName("AI (3902) with a valid value parses and is valid")
    void validElementParsed() {
        ParseResult resp = parser.parse(VALID_INPUT);
        assertTrue(resp.isValid(), "A well-formed AI (3902) element string must be valid");
        assertNotNull(resp.getAiObject().get(GS1Constants_AICodes.AI_3902_AMOUNT), "AI (3902) element must be present");
        assertEquals(VALID_VALUE, resp.getAiObject().get(GS1Constants_AICodes.AI_3902_AMOUNT).getValue(),
                "Parsed value must equal the input value");
    }

    @Test
    @DisplayName("AI (3902) produces interpretations (INTERPRETATION mode)")
    void interpretationsPresent() {
        ParseResult resp = parser.parse(VALID_INPUT);
        assertTrue(resp.isValid(), "Input must be valid for interpretations to be produced");
        assertFalse(resp.getAiObject().get(GS1Constants_AICodes.AI_3902_AMOUNT).getInterpretations().isEmpty(),
                "AI (3902) must produce at least one interpretation in INTERPRETATION mode");
    }

    @Test
    @DisplayName("AI (3902) with no value produces an error")
    void missingValueProducesError() {
        ParseResult resp = parser.parse(GS1Constants_AICodes.AI_3902_AMOUNT);
        assertFalse(resp.isValid(),
                "Parsing AI (3902) with no value must produce at least one error");
    }

    @Test
    @DisplayName("AI (3902) with maximum-length value is valid")
    void maximumLengthValueIsValid() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_255_GCN + "9506000134352"
                + GS1Constants.FNC1_GS
                + GS1Constants_AICodes.AI_3902_AMOUNT + "1".repeat(15));
        assertTrue(resp.isValid(), "A maximum-length value (15 chars) must be valid");
        assertEquals("1".repeat(15), resp.getAiObject().get(GS1Constants_AICodes.AI_3902_AMOUNT).getValue(),
                "Parsed value must equal the maximum-length input");
    }

    @Test
    @DisplayName("AI (3902) with over-maximum-length value produces an error")
    void overMaximumLengthValueProducesError() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_255_GCN + "9506000134352"
                + GS1Constants.FNC1_GS
                + GS1Constants_AICodes.AI_3902_AMOUNT + "1".repeat(16));
        assertFalse(resp.isValid(),
                "A value longer than the declared maximum (16 chars) must produce an error");
    }

    @Test
    @DisplayName("AI (3902) with an out-of-charset character produces an error")
    void invalidCharacterProducesError() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_255_GCN + "9506000134352"
                + GS1Constants.FNC1_GS
                + GS1Constants_AICodes.AI_3902_AMOUNT + "#");
        assertFalse(resp.isValid(),
                "A value containing a character outside the AI's character set must produce an error");
    }

    @Test
    @DisplayName("AI (3902) without its required AIs produces GE-S005")
    void missingRequiredAisProducesError() {
        ParseResult resp = parser.parse(GS1Constants_AICodes.AI_3902_AMOUNT + VALID_VALUE);
        assertFalse(resp.isValid(),
                "AI (3902) without its required AIs must produce a required-AI error");
        assertTrue(resp.getErrors().stream().anyMatch(e -> "GE-S005".equals(e.getId())),
                "The error must be GE-S005 (missing required AI)");
    }

    @Test
    @DisplayName("AI (3902) paired with excluded AI (3900) produces GE-S006")
    void excludedPairingProducesError() {
        ...
        assertTrue(resp.getErrors().stream().anyMatch(e -> "GE-S006".equals(e.getId())),
                "Pairing AI (3902) with excluded AI (3900) must produce GE-S006");
    }

    @Test
    @DisplayName("AI (3902) terminated by a trailing FNC1 is valid with an advisory warning")
    ...
    @Test
    @DisplayName("Fixed-length AI (01) followed by FNC1 then AI (3902) produces a syntax error")
    void fixedLengthAiWithFnc1BeforeThisAiProducesError() {
        // A FNC1 after fixed-length AI (01) mid-stream triggers GE-S011.
        // The parser stops at the FNC1 and AI (3902) is never reached.
        ParseResult resp = parser.parse(
