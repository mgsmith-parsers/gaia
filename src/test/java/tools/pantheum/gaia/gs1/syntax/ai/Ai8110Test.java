 ...
 * Tests for AI (8110).
 *
 * <p>Coupon code identification for use in North America
 *
...
class Ai8110Test {
    ...
    /** A well-formed value for AI (8110). */
    private static final String VALID_VALUE = "1950600013435210123456";

    /** A fully valid element string. */
    private static final String VALID_INPUT =
            GS1Constants_AICodes.AI_8110 + "1950600013435210123456";

    @Test
    @DisplayName("AI (8110) with a valid value parses and is valid")
    void validElementParsed() {
        ParseResult resp = parser.parse(VALID_INPUT);
        assertTrue(resp.isValid(), "A well-formed AI (8110) element string must be valid");
        assertNotNull(resp.getAiObject().get(GS1Constants_AICodes.AI_8110), "AI (8110) element must be present");
        assertEquals(VALID_VALUE, resp.getAiObject().get(GS1Constants_AICodes.AI_8110).getValue(),
                "Parsed value must equal the input value");
    }

    @Test
    @DisplayName("AI (8110) with no value produces an error")
    void missingValueProducesError() {
        ParseResult resp = parser.parse(GS1Constants_AICodes.AI_8110);
        assertFalse(resp.isValid(),
                "Parsing AI (8110) with no value must produce at least one error");
    }

    @Test
    @DisplayName("AI (8110) with maximum-length value is valid")
    void maximumLengthValueIsValid() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_8110 + "A".repeat(70));
        assertTrue(resp.isValid(), "A maximum-length value (70 chars) must be valid");
        assertEquals("A".repeat(70), resp.getAiObject().get(GS1Constants_AICodes.AI_8110).getValue(),
                "Parsed value must equal the maximum-length input");
    }

    @Test
    @DisplayName("AI (8110) with over-maximum-length value produces an error")
    void overMaximumLengthValueProducesError() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_8110 + "A".repeat(71));
        assertFalse(resp.isValid(),
                "A value longer than the declared maximum (71 chars) must produce an error");
    }

    @Test
    @DisplayName("AI (8110) with an out-of-charset character produces an error")
    void invalidCharacterProducesError() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_8110 + "#950600013435210123456");
        assertFalse(resp.isValid(),
                "A value containing a character outside the AI's character set must produce an error");
    }

    @Test
    @DisplayName("AI (8110) terminated by a trailing FNC1 is valid with an advisory warning")
    ...
    @Test
    @DisplayName("Fixed-length AI (01) followed by FNC1 then AI (8110) produces a syntax error")
    void fixedLengthAiWithFnc1BeforeThisAiProducesError() {
        // A FNC1 after fixed-length AI (01) mid-stream triggers GE-S011.
        // The parser stops at the FNC1 and AI (8110) is never reached.
        ParseResult resp = parser.parse(
