...
class Ai97Test {
    ...
    /** A well-formed value for AI (97). */
    private static final String VALID_VALUE = "A";

    /** A fully valid element string. */
    private static final String VALID_INPUT =
            GS1Constants_AICodes.AI_97_INTERNAL + "A";

    @Test
    @DisplayName("AI (97) with a valid value parses and is valid")
    void validElementParsed() {
        ParseResult resp = parser.parse(VALID_INPUT);
        assertTrue(resp.isValid(), "A well-formed AI (97) element string must be valid");
        assertNotNull(resp.getAiObject().get(GS1Constants_AICodes.AI_97_INTERNAL), "AI (97) element must be present");
        assertEquals(VALID_VALUE, resp.getAiObject().get(GS1Constants_AICodes.AI_97_INTERNAL).getValue(),
                "Parsed value must equal the input value");
    }

    @Test
    @DisplayName("AI (97) with no value produces an error")
    void missingValueProducesError() {
        ParseResult resp = parser.parse(GS1Constants_AICodes.AI_97_INTERNAL);
        assertFalse(resp.isValid(),
                "Parsing AI (97) with no value must produce at least one error");
    }

    @Test
    @DisplayName("AI (97) with maximum-length value is valid")
    void maximumLengthValueIsValid() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_97_INTERNAL + "A".repeat(90));
        assertTrue(resp.isValid(), "A maximum-length value (90 chars) must be valid");
        assertEquals("A".repeat(90), resp.getAiObject().get(GS1Constants_AICodes.AI_97_INTERNAL).getValue(),
                "Parsed value must equal the maximum-length input");
    }

    @Test
    @DisplayName("AI (97) with over-maximum-length value produces an error")
    void overMaximumLengthValueProducesError() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_97_INTERNAL + "A".repeat(91));
        assertFalse(resp.isValid(),
                "A value longer than the declared maximum (91 chars) must produce an error");
    }

    @Test
    @DisplayName("AI (97) with an out-of-charset character produces an error")
    void invalidCharacterProducesError() {
        ParseResult resp = parser.parse(
                GS1Constants_AICodes.AI_97_INTERNAL + "#");
        assertFalse(resp.isValid(),
                "A value containing a character outside the AI's character set must produce an error");
    }

    @Test
    @DisplayName("AI (97) terminated by a trailing FNC1 is valid with an advisory warning")
    ...
    @Test
    @DisplayName("Fixed-length AI (01) followed by FNC1 then AI (97) produces a syntax error")
    void fixedLengthAiWithFnc1BeforeThisAiProducesError() {
        // A FNC1 after fixed-length AI (01) mid-stream triggers GE-S011.
        // The parser stops at the FNC1 and AI (97) is never reached.
        ParseResult resp = parser.parse(
